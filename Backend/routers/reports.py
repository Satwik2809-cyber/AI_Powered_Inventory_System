...
def finalize_monthly_count(session: Session = Depends(get_db)):
    ...
    # 5. Transaction Log
    transaction_log = []
    def add_tx_log_final(sale_obj, is_event):
        amt = safe_float(sale_obj.total_amount)
        c = amt if sale_obj.payment_mode == 'cash' else 0
        o = amt if sale_obj.payment_mode != 'cash' else 0
        it_strs = []
    ...
    for s in sales: add_tx_log_final(s, False)
    for es in event_sales: add_tx_log_final(es, True)
    ...
    return {"message": "Monthly count finalized and locked", "period_end": end_date}
